...
# 업로드된 무매 엑셀의 'DB' / '수익 일지' 시트와 동일한 구조
DB_HEADER = ["날짜", "전략명", "모드", "종가($)", "평단가($)", "보유수량", "T값",
             "P값(⭐%)", "5일 평균(A)", "매입금액($)", "잔금($)", "1회 매수금($)",
             "실현손익($)", "매수주문", "매도 1 (쿼터)", "매도 2 (지정가)"]
DIARY_HEADER = ["종료 날짜", "종목", "시작 원금", "종료 금액", "수익금($)",
                "수익률(%)", "최종 T값"]

_HEADERS = {"주문": ORDER_HEADER, "체결": FILL_HEADER, "DB": DB_HEADER,
            "수익 일지": DIARY_HEADER}
